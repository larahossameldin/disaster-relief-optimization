...
class Island:
    def __init__(self, population, scenario, operator, island_id=0, init_strategy="Random"):
        ...
        # Modified to use random initialization by default
        self._ga_helper = DisasterReliefGA(
            scenario_data=scenario,
            config_type="config1",
            init_strategy=init_strategy,  # Now uses the passed strategy (default Random)
...
class DIMSPHybrid:
    def __init__(
        ...
        cluster_strategy="large_psi",  # New parameter for experiment type
        verbose=False,
    ):
        self.scenario = scenario
        ...
        self.cluster_strategy = cluster_strategy  # "large_psi", "small_psi", or "random"
        self.verbose = verbose
        
        # Use the initialize_population pattern with default random
        if init_strategy == "Demand_Proportional":
            pop = initialise_demand_proportional(island_size, scenario, RANDOM_SEED)
        elif init_strategy == "Urgency_Biased":
            pop = initialise_urgency_biased(island_size, scenario, RANDOM_SEED)
        else:  # Default to Random
            pop = initialise_random(island_size, scenario, RANDOM_SEED)
        
        self.islands = [Island(pop, scenario, operator="PSO", island_id=0, init_strategy=init_strategy)]
    ...
    def _determine_operator(self, cluster_size, median_size):
        """Determine which operator to use based on cluster strategy"""
        if self.cluster_strategy == "large_psi":
            # Original: large clusters use PSO, small clusters use GA
    ...
    def _run_epoch(self, current_gen): # Re-clusters population and dynamically assigns GA or PSO to new islands
        ...
        median_size = np.median([len(p) for p in new_pops]) # Computes median cluster size to classify islands
        new_islands = []
        for idx, pop in enumerate(new_pops):
            # Use the strategy-based operator assignment
            op = self._determine_operator(len(pop), median_size)
            isl = Island(pop, self.scenario, operator=op, island_id=idx, 
                        init_strategy=self.islands[0]._ga_helper.init_strategy if self.islands else "Random")
...
def run_experiment(scenario, cluster_strategy, experiment_name):
    """Run a single experiment with specified cluster strategy"""
    print(f"\n{'='*60}")
    ...
    dimsp = DIMSPHybrid(scenario, total_generations=100, epoch_interval=20,
                        ...
                        init_strategy="Random", cluster_strategy=cluster_strategy)
    sol_h, score_h, hist_h = dimsp.run()
    
    ga = DisasterReliefGA(scenario_data=scenario, config_type="config1",
                          init_strategy="Random",  # Changed to Random
    ...
    pso = PSO(scenario=scenario, num_particles=50, max_iterations=100,
              ...
              initialization_strategy="random")  # Changed to random
    ...
    return {
        ...
        "convergence": hist_h["hybrid_convergence"],
        "island_counts": hist_h["island_count"]
    }

if __name__ == "__main__": # Runs three experiments with different clustering strategies
    ...
    experiments = [
        ("Baseline (Large clusters use PSO)", "large_psi"),
        ("Opposite (Small clusters use PSO)", "small_psi"),
    ...
    for exp_name, strategy in experiments:
        result = run_experiment(scenario, strategy, exp_name)
        results.append(result)
    
    # Print comparative summary
    print("\n" + "="*80)
    print("COMPARATIVE SUMMARY OF ALL EXPERIMENTS")
    print("="*80)
    print(f"{'Experiment':<35} {'Hybrid Score':<15} {'Improvement':<15} {'Best Baseline'}")
    print("-"*80)
    ...
    for result in results:
        best_baseline = min(result['ga_score'], result['pso_score'])
        print(f"{result['experiment']:<35} {result['hybrid_score']:<15.4f} "
              f"{result['improvement']:>+6.2f}%-15 {best_baseline:<15.4f}")
    ...
    print("="*80)
    print(f"\nBest performing strategy: {best_overall['experiment']}")
    print(f"Best improvement: {best_overall['improvement']:.2f}%")
    
    # Recommend best strategy
    if best_overall['strategy'] == "large_psi":
        print("\nRecommendation: The original strategy (large clusters using PSO) performed best")
    elif best_overall['strategy'] == "small_psi":
        print("\nRecommendation: The opposite strategy (small clusters using PSO) performed best")
    else:
        print("\nRecommendation: Random assignment of operators performed best")